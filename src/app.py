...
app.layout = dbc.Container(
    [
        ...
        dcc.Store(
            id="card-order", data=["edlap", "sapbw", "tableau", "alteryx"], storage_type="local"
        ),
                    ...
                    [
                        html.Div(
                            [
                                ...
                                # (shown vertically in order of selection)
                                html.Div(
                            ...
                            [
                                ...
                                html.Div(
                                    [
                                        ...
                                        dbc.Button(
                                            html.I(
                                                className="fa fa-sort-amount-down", id="sort-icon"
                                            ),
        ...
        dbc.Alert(
            [
                ...
                "Click any platform card to filter tickets. ",
                "Status thresholds (Healthy/Attention/Critical) ",
...
@callback(Output("summary-bar", "children"), Input("selected-platform", "data"))
    ...
    for selected_platform_id in selected_platforms:
        ...
        if not perf_data:
            drilldown_cards.append(
                html.Div(
                    f"No performance data available for {platform_name}", className="text-muted p-3"
                )
            )
            continue
            ...
            dbc.Card(
                [
                    ...
                    dbc.CardBody(
                        [
                            create_performance_drilldown(
                                selected_platform_id, perf_data, selected_machine
                            )
                        ]
                    ),
                ],
    ...
    if sort_field:
        ...
        if sort_field == "priority":
            ...
            filtered_tickets = sorted(
                filtered_tickets,
                key=lambda t: (
                    t.get(sort_field, "").lower()
                    if isinstance(t.get(sort_field), str)
                    else t.get(sort_field, "")
                ),
                reverse=not is_ascending,
            )
